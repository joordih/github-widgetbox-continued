...
async function profileWidget(username, data, themeString) {
    ...
    if (dataOptions === undefined) {
        return (0, error_1.default)('Profile', '-25%', 'Data option is missing!', '-25%');
    }
    if (dataOptions.length > 4) {
        return (0, error_1.default)('Profile', '-25%', `Can't have more than 4 data-options!`, '-40%');
    }
    const width = 842;
    const height = 165;
    async function getDataOptions() {
            ...
            switch (dataOptions[i].toLowerCase()) {
                ...
                case 'commits':
                    addDataBox('contributions', i, profile.data.user.contributionsCollection
                        .contributionCalendar.totalContributions, '#C5FFD9', '#00F14F', `<g transform="translate(-71 9)">
              ...
              <path d="M12.438,14.87v5.148H10.722V14.87H8.148l3.432-4.29,3.432,4.29Zm1.716,1.716h2.574V14.012h-.686L11.58,8.435,6.987,14.012H6a1.287,1.287,0,0,0,0,2.574h3V18.3H6a3,3,0,0,1-3-3V4.574A2.574,2.574,0,0,1,5.574,2H17.586a.858.858,0,0,1,.858.858V17.444a.858.858,0,0,1-.858.858H14.154ZM6.432,4.574V6.29H8.148V4.574Zm0,2.574V8.864H8.148V7.148Z" transform="translate(-0.426 -0.284)" fill="#00F14F"/>
                    ...
                    break;
                default:
                    return (0, error_1.default)('Profile', '-25%', `Invalid data item found!`, '-26%');
            }
        }
        ...
        <text id="${name}-text" transform="translate(${name === 'followers' ? '-43' : '-47'} 25)" fill="${color2}" font-size="16" font-family="Roboto-Regular, Roboto, sans-serif">
    ...
    try {
      ...
      <text class="title" transform="translate(145 78)" font-size="26" font-family="Roboto-Medium, Roboto, sans-serif" font-weight="500">${(_a = response.data.name) !== null && _a !== void 0 ? _a : response.data.login}</text>
      <text class="subtitle" transform="translate(145 102)" font-size="16" font-family="Roboto-Regular, Roboto, sans-serif">GitHub.com/${response.data.login}</text>
      <g transform="translate(${width - 52} ${(height - 37) / 2})">${dataBoxes}</g>
    </svg>`;
    }
    catch (error) {
